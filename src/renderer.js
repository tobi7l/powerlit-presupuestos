...
// fmtMoney() viene de ticketTemplate.js, cargado antes que este script.
...
// aplicarDescuentos() viene de pricing.js, cargado antes que este script.
...
function buildTicketHTML() {
  ...
  return construirTicketHTML({
    cliente, direccion, telefono, fecha,
    filas: filas.map(f => ({
      cant: f.cant,
      etiqueta: etiquetaProducto(f.producto),
      precioUnitario: precioDe(f.producto),
      importe: f.cant * precioDe(f.producto)
    })),
    subtotal, descMonto, total,
    etiquetaDescuento: etiqueta,
    mostrarDescuento: !modoMinorista || descMonto > 0,
